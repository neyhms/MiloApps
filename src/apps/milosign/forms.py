...
class DocumentUploadForm(FlaskForm):
    ...
    title = StringField(
        "Título del documento", validators=[DataRequired(), Length(min=1, max=200)]
    )
    ...
    file = FileField(
        ...
        validators=[
            FileRequired(),
            FileAllowed(["pdf", "doc", "docx"], "Solo archivos PDF, DOC y DOCX"),
        ],
    )
    ...
    requires_all_signatures = BooleanField("Requiere todas las firmas", default=True)
...
class AddSignerForm(FlaskForm):
    ...
    email = StringField("Email del firmante", validators=[DataRequired(), Email()])
    name = StringField(
        "Nombre completo", validators=[DataRequired(), Length(min=1, max=100)]
    )
    order_index = SelectField(
        "Orden de firma",
        choices=[(str(i), f"Posición {i}") for i in range(1, 11)],
        default="1",
    )
    notes = TextAreaField("Notas para el firmante", validators=[Length(max=500)])
...
class SignatureForm(FlaskForm):
    ...
    agree_terms = BooleanField(
        "Acepto los términos y condiciones", validators=[DataRequired()]
    )
    notes = TextAreaField("Notas adicionales", validators=[Length(max=500)])
...
class TemplateForm(FlaskForm):
    ...
    name = StringField(
        "Nombre de la plantilla", validators=[DataRequired(), Length(min=1, max=200)]
    )
    description = TextAreaField("Descripción", validators=[Length(max=1000)])
    ...
    default_expiration_days = SelectField(
    ...
    requires_all_signatures = BooleanField(
        "Requiere todas las firmas por defecto", default=True
    )
