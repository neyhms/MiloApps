...
milosign_bp = Blueprint('milosign', __name__, 
...
def dashboard():
    ...
    pending_signatures = Document.query.join(Signature).filter(
    ...
    return render_template('milosign/dashboard.html',
...
def upload_document():
    ...
    if form.validate_on_submit():
        ...
        if file:
            ...
            log_audit('DOCUMENT_UPLOADED', 'milosign', current_user.id, 
...
def document_detail(id):
    ...
    return render_template('milosign/document_detail.html',
                         document=document,
...
def sign_document(document_id):
    ...
    if form.validate_on_submit():
        ...
        log_audit('DOCUMENT_SIGNED', 'milosign', current_user.id, 
                 'document', document_id)
        
        flash('Documento firmado correctamente.', 'success')
        return redirect(url_for('milosign.document_detail', id=document_id))
    
    return render_template('milosign/sign.html', 
                         document=document,
                         signature=signature,
                         form=form)
