...
class User(UserMixin, db.Model):
    ...
    def get_2fa_qr_code(self, app_name="MiloApps"):
        ...
        totp_uri = pyotp.totp.TOTP(self.two_factor_secret).provisioning_uri(
            name=self.email, issuer_name=app_name
        )
    ...
    def has_app_permission(self, app_name):
        ...
        return any(
            perm.app_name == app_name and perm.is_active for perm in self.user_apps
        )
    ...
    if admin:
        for app_data in apps_config:
            existing_perm = UserAppPermission.query.filter_by(
                user_id=admin.id, app_name=app_data["app_name"]
            ).first()
            if not existing_perm:
                perm = UserAppPermission(
                    user_id=admin.id, app_name=app_data["app_name"], role="admin"
                )
                db.session.add(perm)
