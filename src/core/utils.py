...
def log_audit(action, app_name, user_id=None, resource_type=None, 
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not current_user.has_app_permission(app_name):
                ...
                return {'error': 'No tienes permisos para acceder a esta aplicación'}, 403
            ...
            if required_role != 'user':
                ...
                if not _check_role_hierarchy(user_role, required_role):
                    log_audit(
                        ...
                        app_name=app_name,
                        user_id=current_user.id,
                        details={'reason': 'Insufficient role', 'required_role': required_role, 'user_role': user_role}
                    )
                    return {'error': f'Necesitas rol de {required_role} o superior'}, 403
...
def get_user_apps(user=None):
    ...
    all_apps = AppConfig.query.filter_by(is_active=True).order_by(AppConfig.order_index).all()
...
def detect_app_from_request():
    ...
    main_domain = current_app.config.get('MILOAPPS_CONFIG', {}).get('main_domain', 'localhost:3000')
    
    if current_app.config.get('MILOAPPS_CONFIG', {}).get('enable_subdomains', False):
        if host != main_domain:
            # Extraer subdominio
            subdomain = host.replace(f".{main_domain}", "").replace(f":{main_domain.split(':')[1]}", "")
            return subdomain
